...
function appendData(sixGB_RAM_Data) {
  // console.log("insideFun");
  container.innerHTML = null;
  sixGB_RAM_Data.forEach((el) => {
...
document
  ...
  .addEventListener("click", sortByRecommended);

function sortByRecommended() {
  sixGB_RAM_Data.sort((a, b) => {
    return Number(a.goodsSn) - Number(b.goodsSn);
  });
  // console.log(sixGB_RAM_Data);
  appendData(sixGB_RAM_Data);
}
...
function sortByHottest() {
  sixGB_RAM_Data.sort(function (a, b) {
    return Number(b.goodsSn) - Number(a.goodsSn);
  });
  appendData(sixGB_RAM_Data);
}

document.querySelector("#newest").addEventListener("click", sortByNewest);
