...
appendData(waterproofPhoneData);

const wishedArr = JSON.parse(localStorage.getItem("wishedItem")) || [];

function appendData(waterproofPhoneData) {
  // console.log("insideFun");
  container.innerHTML = null;
  waterproofPhoneData.forEach((el) => {
...
function sortByRecommended() {
  waterproofPhoneData.sort((a, b) => {
    return (Number(a.goodsSn) - Number(b.goodsSn));
  })
  // console.log(waterproofPhoneData);
  appendData(waterproofPhoneData);
};
...
function sortByHottest() {
  waterproofPhoneData.sort(function (a, b) {
