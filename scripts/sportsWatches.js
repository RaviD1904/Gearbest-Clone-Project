...
appendData(sportsWatchesData);
const wishedArr = JSON.parse(localStorage.getItem("wishedItem")) || [];

function appendData(sportsWatchesData) {
  // console.log("insideFun");
  container.innerHTML = null;
  sportsWatchesData.forEach((el) => {
...
function sortByRecommended() {
  sportsWatchesData.sort((a, b) => {
    return (Number(a.goodsSn) - Number(b.goodsSn));
  })
  // console.log(sportsWatchesData);
  appendData(sportsWatchesData);
};
...
function sortByHottest() {
  sportsWatchesData.sort(function (a, b) {
